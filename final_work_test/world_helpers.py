...
class WorldHelper:
    ...
    def executeUPFPlan(self, plan: Any | None) -> None:
        ...
        for i, action in enumerate(plan.actions):
            ...
            if action_name == "move":
                ...
                task_list.append(
                    TaskAction("place", object=target_object_name, target_location=target_location_name)
                )
                #task_list.append(TaskAction("detect", object=target_object_name))
    ...
    def generateProblemPDDL(self, problem_for_pddl: str, domain_for_pddl: str) -> str:
        ...
        def generateGoal(target_loc: str = None) -> str:
            ...
            for object in objects:
                if object.name == "food":
                    target_location = "table0"
                elif object.name == "toothbrush":
                    target_location = "counter0"
                elif object.name == "cosmetic":
                    target_location = "counter0"
        ...
        goal_for_pddl = generateGoal(target_loc="trash_can")
