...
\begin{abstract}
We propose a \textit{PyRoboSim}-based mobile robot that \emph{continuously} tidies a domestic environment by fetching clutter (dirty dishes, laundry, small toys) from their spawning locations and depositing each item in its designated receptacle.  A script-generated PDDL domain captures the pick--navigate--place loop, enabling a symbolic planner (Fast Downward) to produce high-level task sequences on demand.  Motion is decomposed into three layers: (i) a global \textbf{RRT*} path computed with OMPL to connect semantic way-points across rooms; (ii) a real-time grid-based \textbf{A*} planner that refines each leg of the route; and (iii) a reactive \textbf{Bug2} controller that guarantees escape from unforeseen blockages.  Because PyRoboSim supplies ground-truth object coordinates, no external perception module is required, allowing us to focus on planning, control, and life-cycle integration under ROS~2.
\end{abstract}
